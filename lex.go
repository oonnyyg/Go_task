...
// errorPrevLine is like error(), but sets the position to the last column of
// the previous line.
//
// This is so that unexpected EOF or NL errors don't show on a new blank line.
// When the input ends without a trailing newline there is no blank line to
// move back from, so the error stays on the current line.
func (lx *lexer) errorPrevLine(err error) stateFn {
	lx.items <- item{typ: itemError, pos: lx.prevLinePos(), err: err}
	return nil
}

// prevLinePos returns the position for errors reported at the end of the
// previous line, such as unexpected EOF or a newline where it's not allowed.
//
// The line is moved back only when a newline was actually crossed: at EOF
// after a trailing newline, or right after consuming a newline. When the
// input ends without a trailing newline the error stays on the current line,
// which keeps it from dropping below line 1.
func (lx *lexer) prevLinePos() Position {
	pos := lx.getPos()
	if lx.pos > 0 && lx.input[lx.pos-1] == '\n' {
		pos.Line--
	}
	pos.Len = 1
	pos.Start = lx.pos - 1
...
func (lx *lexer) errorf(format string, values ...any) stateFn {
	if lx.atEOF {
		lx.items <- item{typ: itemError, pos: lx.prevLinePos(), err: fmt.Errorf(format, values...)}
		return nil
	}
