...
func customOptions(opts Options) Options {
	...
	// A custom Environment is the sole source of env values and must replace
	// os.Environ() entirely instead of being merged on top of it, so that keys
	// absent from the custom map are treated as missing (or fall back to defaults).
	if opts.Environment != nil {
		defOpts.Environment = opts.Environment
	}
	return defOpts
}
