...
func toEnvName(input string) string {
	...
	appendRune := func(c rune) {
		if c == underscore && len(output) > 0 && output[len(output)-1] == underscore {
			return
		}
	...
	for i, c := range runes {
		...
		appendRune(unicode.ToUpper(c))
	}
	return strings.Trim(string(output), string(underscore))
}
