			...
			case reflect.Map:
				...
				for _, key := range current.MapKeys() {
					...
					if ct != nil && ct.typeof == typeKeys && ct.keys != nil {
						...
						// even when only keys are validated (ct.next == nil) the value must still be
						// traversed so struct values are validated via validateStruct, consistent with
						// slices and the non-keys map branch; traverseField is a no-op for non-struct
						// values when there is no value rule.
