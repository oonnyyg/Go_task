...
func (p *parser) setValue(key string, value any) {
	...
	if _, ok := hash[key]; ok {
		...
		if p.isImplicit(keyContext) {
			p.removeImplicit(keyContext)
			// It has already been created as a table; only a table may redef. it
			// (e.g. [a.b] after a.b.c = 1). Any other type is a conflict and must
			// be reported as an error instead of silently discarding the value.
