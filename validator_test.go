...
func TestKeysMapStructValue(t *testing.T) {
	type Item struct {
		Name string `validate:"required"`
	...
	type Test struct {
		// keys-only validation: no value rule follows endkeys, so previously the
		// struct values were never traversed and invalid fields were silently ignored.
		M  map[string]Item  `validate:"dive,keys,required,endkeys"`
		MP map[string]*Item `validate:"dive,keys,required,endkeys"`
	}

	validate := New()

	// valid values -> no errors; a nil pointer value has no value rule and must not error
	tst := Test{
		M: map[string]Item{
			"goodkey": {Name: "ok", Age: 20},
		},
		MP: map[string]*Item{
			"goodkey": {Name: "ok", Age: 20},
			"nilkey":  nil,
	...
	// invalid struct value -> fields must be reported
	tst.M["badkey"] = Item{Name: "", Age: 5}
	tst.MP["badptrkey"] = &Item{Name: "", Age: 5}
	...
	AssertDeepError(t, errs, "Test.M[badkey].Name", "Test.M[badkey].Name", "Name", "Name", "required", "required")
	AssertDeepError(t, errs, "Test.M[badkey].Age", "Test.M[badkey].Age", "Age", "Age", "gte", "gte")
	AssertDeepError(t, errs, "Test.MP[badptrkey].Name", "Test.MP[badptrkey].Name", "Name", "Name", "required", "required")
	AssertDeepError(t, errs, "Test.MP[badptrkey].Age", "Test.MP[badptrkey].Age", "Age", "Age", "gte", "gte")

	// a bad key must still be reported alongside the value validation
	tst.M[""] = Item{Name: "ok", Age: 20}
	err = validate.Struct(tst)
	NotEqual(t, err, nil)
	AssertDeepError(t, err.(ValidationErrors), "Test.M[]", "Test.M[]", "M[]", "M[]", "required", "required")

	// nested struct holding a keys-only map must also surface value errors
	type Nested struct {
		M map[string]Item `validate:"dive,keys,required,endkeys"`
	}
	type Outer struct {
	...
	nerr := validate.Struct(Outer{Nested: Nested{M: map[string]Item{
		"badkey": {Name: "", Age: 5},
	}}})
	NotEqual(t, nerr, nil)
	nerrs := nerr.(ValidationErrors)
	Equal(t, len(nerrs), 2)
	AssertDeepError(t, nerrs, "Outer.Nested.M[badkey].Name", "Outer.Nested.M[badkey].Name", "Name", "Name", "required", "required")
	AssertDeepError(t, nerrs, "Outer.Nested.M[badkey].Age", "Outer.Nested.M[badkey].Age", "Age", "Age", "gte", "gte")

	// non-struct values with keys-only must NOT produce spurious value errors
	NotEqual(t, validate.Var(map[string]string{"": "anything"}, "dive,keys,required,endkeys"), nil)
	serrs := validate.Var(map[string]string{"": "anything"}, "dive,keys,required,endkeys").(ValidationErrors)
	Equal(t, len(serrs), 1)
	Equal(t, serrs[0].Tag(), "required")
	Equal(t, validate.Var(map[string]string{"goodkey": ""}, "dive,keys,required,endkeys"), nil)
}
