...
func TestCompletionTraverseChildrenNoArgsMutation(t *testing.T) {
	rootCmd := &Command{
		Use:              "root",
		Run:              emptyRun,
	...
	var seenFinalArgs []string
	var seenToComplete string
	subCmd := &Command{
		Use:               "sub",
		Run:               emptyRun,
		ValidArgsFunction: func(cmd *Command, args []string, toComplete string) ([]string, ShellCompDirective) {
			seenFinalArgs = append([]string{}, args...)
			seenToComplete = toComplete
			return []string{"one", "two"}, ShellCompDirectiveNoFileComp
		},
	}
	rootCmd.AddCommand(subCmd)

	// Simulate the shell invoking: root __complete sub arg1 <TAB>.
	// Keep a reference to args so mutation of the caller's backing array
	// (which aliases os.Args in a real process) is observable.
	args := []string{ShellCompRequestCmd, "sub", "arg1", ""}
	if _, err := executeCommand(rootCmd, args...); err != nil {
		t.Fatalf("Unexpected error: %v", err)
	}

	// The completion should have reached the subcommand's ValidArgsFunction.
	if seenToComplete != "" {
		t.Errorf("Expected empty toComplete, got %q", seenToComplete)
	}
	if strings.Join(seenFinalArgs, ",") != "arg1" {
		t.Errorf("Expected finalArgs [arg1], got %v", seenFinalArgs)
	}

	// Completion must only read its input; it must not pollute the caller's
	// argument slice (the original "" must not be rewritten to "--").
	want := []string{ShellCompRequestCmd, "sub", "arg1", ""}
	if strings.Join(args, "\x00") != strings.Join(want, "\x00") {
		t.Errorf("completion mutated caller args:\nexpected: %v\ngot:      %v", want, args)
	}
}
