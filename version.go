...
func NewVersion(v string) (*Version, error) {
	...
	var err error
	// Numeric identifiers MUST NOT include leading zeroes. The regex used
	// for parsing permits them so they are explicitly validated here.
	...
	if m[2] != "" {
		...
		sv.minor, err = strconv.ParseUint(minor, 10, 64)
		if err != nil {
			return nil, fmt.Errorf("Error parsing version segment: %s", err)
		}
	} else {
	...
	if m[3] != "" {
		...
		sv.patch, err = strconv.ParseUint(patch, 10, 64)
		if err != nil {
			return nil, fmt.Errorf("Error parsing version segment: %s", err)
		}
	} else {
