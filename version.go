...
func (v Version) IncPatch() Version {
	...
	if v.pre != "" {
		...
		vNext.metadata = ""
		vNext.pre = ""
		vNext.patch = incVersionPart(v.patch, "patch")
	}
...
func (v Version) IncMinor() Version {
	...
	vNext.pre = ""
	vNext.patch = 0
	vNext.minor = incVersionPart(v.minor, "minor")
	vNext.original = v.originalVPrefix() + "" + vNext.String()
	return vNext
}
...
func (v Version) IncMajor() Version {
	...
	vNext.patch = 0
	vNext.minor = 0
	vNext.major = incVersionPart(v.major, "major")
	vNext.original = v.originalVPrefix() + "" + vNext.String()
	return vNext
}

// incVersionPart increments a uint64 version segment, panicking when it is
// already at the maximum representable value so incrementing never silently
// overflows back to 0.
func incVersionPart(part uint64, name string) uint64 {
	if part == math.MaxUint64 {
		panic(fmt.Sprintf("semver: cannot increment %s version: already at maximum value", name))
	}
	return part + 1
}
