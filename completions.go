...
func (c *Command) getCompletions(args []string) (*Command, []Completion, ShellCompDirective, error) {
	...
	flagCompletion := true
	// finalArgs may share its backing array with the caller's args (or os.Args
	// when TraverseChildren is set), so build a copy before appending the "--"
	// sentinel; appending in place would overwrite a user argument with "--".
	finalArgsCopy := make([]string, len(finalArgs)+1)
	copy(finalArgsCopy, finalArgs)
	finalArgsCopy[len(finalArgs)] = "--"
	_ = finalCmd.ParseFlags(finalArgsCopy)
