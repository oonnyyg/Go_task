...
// TestNewV7Monotonic verifies that successive NewV7 calls produce UUIDs that
// are strictly increasing in byte order, so they can be sorted as strings or
// byte slices and stay in creation order even within a single millisecond.
func TestNewV7Monotonic(t *testing.T) {
	...
	if prev.Version() != 7 || prev.Variant() != RFC4122 {
		t.Fatalf("bad uuid: version=%s variant=%d", prev.Version(), prev.Variant())
	}
	for i := 1; i < n; i++ {
		cur, err := NewV7()
		if err != nil {
		...
		if cmp := bytes.Compare(prev[:], cur[:]); cmp >= 0 {
			t.Fatalf("NewV7 not strictly increasing at i=%d: prev=%s cur=%s cmp=%d", i, prev, cur, cmp)
		}
		prev = cur
	}
}

// TestNewV7Monotonic_pooled runs the same monotonicity check with the
// randomness pool enabled.
func TestNewV7Monotonic_pooled(t *testing.T) {
	SetRand(nil)
	EnableRandPool()
	defer DisableRandPool()
	const n = 100000
