...
// pow10Int64 returns 10^n for 0 <= n <= 18. The largest result, 10^18,
// still fits in an int64 and is used to verify digit-count estimates for
// integers that are exactly representable as a float64.
...
func (d Decimal) NumDigits() int {
	...
	if v.IsInt64() {
		...
		if i64 <= (1<<53) && i64 >= -(1<<53) {
			...
			// Estimate from the bit length and verify with an exact integer
			// comparison, as done in the big.Int path below. math.Log10 is not
			// used here because it can be off by one for exact powers of ten
			// (e.g. Log10(1e15) returns 14.999999999999998).
			estimatedNumDigits := int(float64(v.BitLen()) / math.Log2(10))
			if absI64 >= pow10Int64(estimatedNumDigits) {
